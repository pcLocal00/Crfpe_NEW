...
@if(isset($datas['sessions']))
    @foreach($datas['sessions'] as $d)
        ...
        <div class="nobreak">
            ...
            <p class="p-title" style="margin-left:2px;">SESSION : <strong>{{$d['code']}} {{$d['title']}}</strong></p>
            ...
            <table class="list" style="margin-bottom:20px;">
                <thead>
                    <tr style="background-color:#F1F1F1;">
                        ...
                        <th class="bordered">Emargement stagiaire <br>@if(array_key_exists("M",$arrayDate['schedules']))
                            {{$arrayDate['schedules']['M']['start_hour']}} - {{$arrayDate['schedules']['M']['end_hour']}}@endif
                        </th>

                        <th class="bordered">Emargement stagiaire <br>@if(array_key_exists("A",$arrayDate['schedules']))
                            {{$arrayDate['schedules']['A']['start_hour']}} - {{$arrayDate['schedules']['A']['end_hour']}}@endif
                        </th>
                        ...
                        <th class="bordered" colspan="2">
                            <table>
                                <thead>
                                    ...
                                    <tr>
                                        <th style="font-weight:normal;">@if(array_key_exists("M",$arrayDate['schedules']))
                                            {{$arrayDate['schedules']['M']['start_hour']}} -
                                            {{$arrayDate['schedules']['M']['end_hour']}}@endif
                                        </th>
                                        <th style="font-weight:normal;">@if(array_key_exists("A",$arrayDate['schedules']))
                                            {{$arrayDate['schedules']['A']['start_hour']}} -
                                            {{$arrayDate['schedules']['A']['end_hour']}}@endif
                                        </th>
                                    </tr>
                                </thead>
                            </table>
...
@php
$member_name = ($member->contact)?($member->contact->firstname.'
'.$member->contact->lastname):$member->unknown_contact_name;

$entitie = ($member->contact && $member->contact->entitie->entity_type == 'S')?$member->contact->entitie->name:($member->unknown_contact_name ?? '');
@endphp
...
<div>
    ...
    <p>Dates : du {{$PARAMS['STARTED_AT']}} au {{$PARAMS['ENDED_AT']}}</p>
    <p>Horaire : de {{$PARAMS['FIRST_SCHEDULE_HOUR']}} à {{$PARAMS['LAST_SCHEDULE_HOUR']}}</p>
    <p>Durée : {{$PARAMS['NB_THEO_DAYS']}} jours / {{$PARAMS['NB_THEO_HOURS']}} heures heures théoriques
        @if($PARAMS['NB_PRACTICAL_HOURS'])
            et {{$PARAMS['NB_PRACTICAL_DAYS']}} jours /{{$PARAMS['NB_PRACTICAL_HOURS']}} heures de stage pratique
        @endif
    </p>
...
<table class="list" id="table_{{$k}}">
    <thead>
        <tr style="background-color:#F1F1F1;">
            <th class="bordered" colspan="8">PARTICIPANT : <strong>{{$member_name}} {{$entitie}}</strong></th>
        </tr>
        <tr style="background-color:#F1F1F1;">
    ...
    <tbody>
        ...
        <tr>
            <td class="bordered">
                ...
                <p class="font-body-table">{{$d['session_id']}} - {{substr($d['session_title'], 0, 10)}}</p>
            </td>
            <td class="bordered font-body-table">
                @if(array_key_exists("M",$d['schedules'])){{$d['schedules']['M']['start_hour']}} -
                {{$d['schedules']['M']['end_hour']}} @endif</td>
            <td class="bordered font-body-table"></td>
            <td class="bordered font-body-table">
                @if(array_key_exists("A",$d['schedules'])){{$d['schedules']['A']['start_hour']}} -
                {{$d['schedules']['A']['end_hour']}} @endif</td>
            <td class="bordered font-body-table"></td>
            <td class="bordered font-body-table"></td>
            <td class="bordered font-body-table">
                @php
