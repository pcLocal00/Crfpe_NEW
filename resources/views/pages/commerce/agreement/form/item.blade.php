...
<div class="modal-body" id="modal_form_agreementitem_body">
    <div data-scroll="true" data-height="600">
        ...
        @endif
        <form id="formAgreementItem" class="form">
            @csrf
            <input type="hidden" name="id" value="{{ ($row)?$row->id:0 }}" />
            <input type="hidden" name="agreement_id" value="{{ ($row)?$row->agreement_id:$agreement_id }}" />
...
<div class="modal-footer">
    <button type="button" class="btn btn-sm btn-light-info" data-dismiss="modal"><i class="fa fa-times"></i>
        Annuler</button>
    <button type="button" onclick="$('#formAgreementItem').submit();" class="btn btn-sm btn-success"><i
            class="fa fa-check"></i> Valider <span id="BTN_SAVE_AGREEMENT_ITEM"></span></button>
</div>
...
$("#formAgreementItem").validate({
    rules: {},
    messages: {},
    submitHandler: function(form) {
        _showLoader('BTN_SAVE_AGREEMENT_ITEM');
        var formData = $(form).serializeArray(); // convert form to array
        $.ajax({
            type: 'POST',
            url: '/form/agreement-item',
            data: formData,
            dataType: 'JSON',
            success: function(result) {
                _hideLoader('BTN_SAVE_AGREEMENT_ITEM');
                if (result.success) {
                    _showResponseMessage('success', result.msg);
                    $('#modal_form_agreementItem').modal('hide');
                } else {
