...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 ">
        <div class="card-title">
            <h3 class="card-label">Gestion des modèles de plannification
            </h3>
        </div>
        <div class="card-toolbar">
            ...
            <button class="btn btn-sm btn-icon btn-light-primary mr-2" data-toggle="tooltip" title="Rafraîchir"
                onclick="_reload_dt_ptemplates()"><i class="flaticon-refresh"></i></button>
            <button class="btn btn-sm btn-icon btn-light-primary" data-toggle="tooltip" title="Ajouter un paramétrage"
                onclick="_formPtemplate(0)"><i class="flaticon2-add-1"></i></button>
            <!--end::Button-->
        </div>
    </div>
    <div class="card-body">
        <!--begin::filter-->
        <x-filter-form type="PTemplates" />
        <!--end::filter-->
        <!--begin: Datatable-->
        <table class="table table-bordered table-checkable" id="dt_ptemplates">
            <thead>
                <tr>
                    <th></th>
                    <th>Modèle</th>
...
<!--end::Card-->
<x-modal id="modal_form_ptemplate" content="modal_form_ptemplate_content" />
...
var dtUrl = '/api/sdt/ptemplates'; 
var table = $('#dt_ptemplates');
// begin first table
table.DataTable({
    language: {
...
var _reload_dt_ptemplates = function() {
    $('#dt_ptemplates').DataTable().ajax.reload();
}
var _formPtemplate = function(row_id) {
    var modal_id = 'modal_form_ptemplate';
    var modal_content_id = 'modal_form_ptemplate_content';
    var spinner = '<div class="modal-body"><div class="spinner spinner-primary spinner-lg"></div></div>';
    $('#' + modal_id).modal('show');
    $('#' + modal_content_id).html(spinner);
    $.ajax({
        url: '/form/ptemplate/' + row_id,
        type: 'GET',
        dataType: 'html',
...
var form_id = 'formFilterPTemplates';
