...
class ContactsImportProspect implements OnEachRow, WithHeadingRow
{
    ...
    public function onRow(Row $row) //model(array $row)
    {
        ...
        if ($nb_contacts == 1) {
            ...
            $contact = $contacts->filter(function ($cn) use ($row) {
                return $row['telephone'] == $cn->pro_phone;
            })->first() ?? null;
            $to_memorise = $contact == null;
        }
        ...
        if ($to_memorise) {
            ...
            $r->log_desc = 'Ligne ' . $rowIndex . ' mémorisée (' . $nb_contacts . ' propositions) - la ligne ' . $rowIndex . ' du fichier xlsx (PROSPECTS)';
            $r->save();
