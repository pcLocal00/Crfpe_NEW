        ...
        if ($type == 'formation') {
            ...
            if ($force_delete == 1) {
                ...
                if(count($ids) > 1)
                {
                    ...
                    {
                        $schedules_contacts = Schedulecontact::where('member_id',$value->id)->withTrashed()->get();

                            // dd($ids,$schedules_contacts);

                            if(count($schedules_contacts) > 0)
                            {
                                foreach($schedules_contacts as $schedule)
                                {
                            ...
                            // dd($schedules_contacts);

                            $searchmember = Member::where('id', $value->id)->first();


                            if(isset($searchmember))
                            {
                                $deletedRows = Member::where('id', $value->id)->forceDelete();
                    ...
                    // dd($ids[0]->id);
                    $schedules_contacts = Schedulecontact::where('member_id',$ids[0]->id)->withTrashed()->get();
                            ...
                            $searchmember = Member::where('id', $ids[0]->id)->first();


                            if(isset($searchmember))
                            {
                                $deletedRows = Member::where('id', $ids[0]->id)->forceDelete();
                ...
                // $member = Member::where('contact_id',$value)->first();
// dd($ids);
    ...
    */
                                ...
                                foreach ($periods as $p) {
                                    ...
                                    foreach (range(1, $nbdecoupage) as $dindex) { //4
                                        foreach (range(1, $nbseance) as $sindex) { //
                                            ...
                                            $end = clone $start;
                                            // dd('hna',$start,$start,$end->addHours($nb_hours));
                                            $dataSchedule = array(
                                                'id' => 0,
                                                'type' => $p->type,
                                                'start_hour' => $start,
        ...
        if ($session_id > 0) {
            ...
            if (count($datesToProgram) > 0) {
                foreach ($datesToProgram as $planning_date) {
                    ...
                    $session_date_id = $this->manageSessiondate($dataSessiondate);
                    // dd($session,$datesToProgram,$session_date_id);
                    ...
                    if ($session_date_id > 0 && $session->planning_template_id > 0) {
                        ...
                        if ($pt->id) {
                            ...
                            if (count($periods) > 0) {
                                foreach ($periods as $p) {
                    // dd($session,$datesToProgram,$session_date_id,$p);
                                    ...
                                    $start = Carbon::createFromFormat('Y-m-d H:i:s', $p->start_hour);
                                    $end = Carbon::createFromFormat('Y-m-d H:i:s', $p->end_hour);
                                    $start_hour = $date->format('Y-m-d') . ' ' . $start->format('H:i:s');
                                    $end_hour = $date->format('Y-m-d') . ' ' . $end->format('H:i:s');

                                    // Having right to schedule two lessons continuous ex from 9 to 12 and from 12 to 13


                                    $schedules_end_hour = Schedule::where('end_hour',$start_hour)->count();


                                    if($schedules_end_hour > 0)
                                    {
                                        $carbonDateTime = Carbon::parse($start_hour);

                                        $seconds = $carbonDateTime->second;

                                        $seconds = $seconds + 1;

                                        $inputDatetime_start = Carbon::parse($start_hour);
                                        $inputDatetime_start->addSeconds($seconds);

                                        $start_hour = $inputDatetime_start->format('Y-m-d H:i:s');

                                        $inputDatetime_end = Carbon::parse($end_hour);
                                        $inputDatetime_end->addSeconds($seconds);

                                        $end_hour = $inputDatetime_end->format('Y-m-d H:i:s');
                                    }
    ...
    public function getMemberSessionPlanningsByMember($sessions_ids, $member_id, $start = '', $end = '')
    {
        ...
        if (count($ids_sessiondates) > 0) {
            if (isset($start) && isset($end) && !empty($start) && !empty($end)) {
                //dd($start);
                $sessiondates = Sessiondate::whereIn('session_id', $sessions_ids)->whereIn('id', $ids_sessiondates)
                ->whereBetween('planning_date', [$start, $end])->orderBy('planning_date', 'asc')->get();
            } else {
    ...
    private function isReserved1($ressource_id, $session_date, $start_hour, $end_hour)
    {
        ...
        // dd($ressource_id, $session_date, $start_hour, $end_hour);
        ...
        // Not allowing for external ressorces
        if($resource->is_internal != 0 && $this->isReserved($ressource_id, $session_date, $start_hour, $end_hour))
        {
    ...
    public function checkIfRessourceAvailableToSchedule($ressource_id, $schedule_id)
    {
        ...
        if ($ressource_id > 0 && $schedule_id > 0) {
            $ressourcename = Ressource::select('name')->where('id', $ressource_id)->pluck('name')->first();
            $schedule = Schedule::find($schedule_id);
            // dd($ressourcename,$schedule);
            if ($schedule) {
                $start_hour = Carbon::createFromFormat('Y-m-d H:i:s', $schedule->start_hour);
                $end_hour = Carbon::createFromFormat('Y-m-d H:i:s', $schedule->end_hour);
                $session_date = $schedule->sessiondate->planning_date;
                //    dd($this->isReserved($ressource_id, $session_date, $start_hour, $end_hour),$this->isReserved1($ressource_id, $session_date, $start_hour, $end_hour));
                // dd($this->isReserved1($ressource_id, $session_date, $start_hour, $end_hour));
                ...
                if ($this->isReserved($ressource_id, $session_date, $start_hour, $end_hour)) {
                    ...
                    if(isset($parent->ressource_id))
                    {
                        ...
                        ->leftJoin('af_actions AS AF', 'AF.id', '=', 'SES.af_id')
                        ->leftJoin('res_ressources AS RES', 'RES.id', '=', 'SRE.ressource_id')
                        ->where('SRE.ressource_id', $father_id)
                        ->whereDate('SDA.planning_date', '=', $session_date)
                        ->where(function ($query) use ($start_hour, $end_hour) {
                            $query->whereBetween('SCH.start_hour', [$start_hour, $end_hour])
                                ->orWhereBetween('SCH.end_hour', [$start_hour, $end_hour]);
                ...
                //         $reserved = Schedule::leftJoin('af_sessiondates', 'af_schedules.sessiondate_id', '=', 'af_sessiondates.id')
                // ->leftJoin('af_scheduleressources', 'af_schedules.id', '=', 'af_scheduleressources.schedule_id')
                // ->where('af_scheduleressources.ressource_id', $ressource_id)
                // ->whereDate('af_sessiondates.planning_date', '=', $session_date)
                // ->where(function ($query) use ($start_hour, $end_hour) {
                //     $query->whereBetween('af_schedules.start_hour', [$start_hour, $end_hour])
                //         ->orWhereBetween('af_schedules.end_hour', [$start_hour, $end_hour]);
                ...
                // ->get();
                    ...
                    if (count($rsScheduleressources) > 0) {
                        ...
                        // dd($rsScheduleressources,$arrayMessages);
                    ...
                    //     ->where('SRE.ressource_id', $ressource_id)
                    //     ->whereDate('SDA.planning_date', '=', $session_date)
                    //     ->whereDate('SCH.start_hour', '=', $start_hour)
                    //     ->whereDate('SCH.end_hour', '=', $end_hour)
                    //     ->where('RES.name', '=', $ressourcename)->get();
                    ...
                    $rsScheduleressources = DB::table('af_scheduleressources AS SRE')
                        ...
                        ->whereDate('SDA.planning_date', '=', $session_date)
                        ->where(function ($query) use ($start_hour, $end_hour) {
                            $query->where(function ($subquery) use ($start_hour, $end_hour) {
                                $subquery->where(function ($innerSubquery) use ($start_hour) {
                                    $innerSubquery->where('SCH.start_hour', '<=', $start_hour)
                                                 ->where('SCH.end_hour', '>=', $start_hour);
                                })
                                ->orWhere(function ($innerSubquery) use ($end_hour) {
                                    $innerSubquery->where('SCH.start_hour', '<=', $end_hour)
                                                 ->where('SCH.end_hour', '>=', $end_hour);
                                });
                            });
                        })->where('RES.name', 'LIKE', $ressourcename)->get();

                    // dd($ressource_id,$ressourcename, $session_date, $start_hour, $end_hour,$rsScheduleressources,$schedule_id);
                    ...
                    //     $session_date = Sessiondate::find($schedule->sessiondate_id);

                    //     $arrayMessages[] = [
                    //         'ressource_id' => $ressource_id,
                    //         'session_id' => $schedule->sessiondate->session->id,
                    //         'sessiondate_id' => $schedule->sessiondate->id,
                    //         'schedule_id' => $schedule_id
                    //     ];
                    // } 
                }else{

                    $new_name = $ressourcename;



                    $allowedPrefixes = array("101", "202", "203", "204");

                    foreach ($allowedPrefixes as $prefix) {
                        if (substr($ressourcename, 0, strlen($prefix)) === $prefix) {
                            $new_name =  $prefix;
                        }
                    }

                    if($ressourcename == "Grand Amphithéâtre" || $ressourcename == "Amphithéâtre haut" || $ressourcename == "Amphithéâtre bas")
                    {
                        $new_name = "Amphithéâtre";
                    }
                    ...
                    //                 $subquery->where(function ($innerSubquery) use ($start_hour) {
                    //                     $innerSubquery->where('SCH.start_hour', '<=', $start_hour)
                    //                                  ->orWhere('SCH.end_hour', '>=', $start_hour);
                    //                 })
                    //                 ->orWhere(function ($innerSubquery) use ($end_hour) {
                    //                     $innerSubquery->where('SCH.start_hour', '<=', $end_hour)
                    //                                  ->orWhere('SCH.end_hour', '>=', $end_hour);
                    //                 });
                    //             });
                        
                    //     })->where('RES.name', 'LIKE', '%'.$new_name.'%')->get();

                    // dd($ressource_id,$new_name);
                    ...
                    ->leftJoin('af_actions AS AF', 'AF.id', '=', 'SES.af_id')
                    ->leftJoin('res_ressources AS RES', 'RES.id', '=', 'SRE.ressource_id')
                    ->where('RES.name', 'LIKE', '%' . $new_name . '%')
                    ->where('SDA.planning_date', '=', $session_date)
                    ->where(function ($query) use ($start_hour, $end_hour) {
                        $query->where(function ($subquery) use ($start_hour, $end_hour) {
                            $subquery->where(function ($innerSubquery) use ($start_hour, $end_hour) {
                                $innerSubquery->where('SCH.start_hour', '<', $end_hour)
                                    ->where('SCH.end_hour', '>', $start_hour);
                            });
                        });
                    ...
                    // dd($new_name,$ressource_id,$rsScheduleressources);


                    // dd($new_name,$rsScheduleressources);



                    // dd($rsScheduleressources,$start_hour,$end_hour,$ressource_id,$session_date);

                //    dd($this->isReserved($ressource_id, $session_date, $start_hour, $end_hour),$this->isReserved1($ressource_id, $session_date, $start_hour, $end_hour),$rsScheduleressources);


                    if (count($rsScheduleressources) > 0) {
                        foreach ($rsScheduleressources as $scheduleressource) {
                            $is_internal = ($scheduleressource && $scheduleressource->type == "RES_TYPE_LIEU") ? $scheduleressource->is_internal : null;
                            $schedule_resource = Scheduleressource::where('ressource_id',$scheduleressource->ressource_id)->where('schedule_id',$scheduleressource->schedule_id)->first();

                            $schedule_resource_other_planif = Scheduleressource::where('schedule_id',$scheduleressource->schedule_id)->first();

                            
                            // dd($is_internal,$schedule_resource_other_planif);

                            // dd('Test',$scheduleressource->id);

                            // dd($ressource_id,$scheduleressource->id,$scheduleressource->ressource_id);

                            if($ressource_id != $scheduleressource->id && $ressource_id != $scheduleressource->ressource_id)
                            {
                                $toschedule = true;
                            }
                            if (in_array($scheduleressource->id, [37, 31, 33, 30]))
                            {
                                $toschedule = true;
                            }
                            ...
                            else if($schedule_resource_other_planif)
                            {
                                $is_available = false;
                                $arrayMessages[] = [
                                    'ressource_id' => $scheduleressource->ressource_id,
                            ...
                            else if (!isset($scheduleressource->ressource_id) || !isset($schedule_resource)) {
                                $toschedule = true;
                            } else {
                                $is_available = false;
                                $arrayMessages[] = [
                            ...
                            // dd($scheduleressource->ressource_id,$schedule_resource,$scheduleressource);
        ...
        //  dd($is_available,$toschedule,$arrayMessages);
                ...
                if ($schedule) {
                    ...
                    if (count($rsScheduleressources) > 0) {
                        foreach ($rsScheduleressources as $sr) {
                            ...
                            // Find Resource 04/08/20023
                            if($sr->ressource->is_internal == 0)
                            {
                                $can_be_scheduled = true;
                            }else{
                                $can_be_scheduled = false;
    ...
    public function generateHtmlPdfAttendanceAbsenceSheet($af_id, $render_type, $group_id, $start_date = '', $end_date = '', $session_id = '', $member_id = '', $training_site = '')
    {
        ...
        if ($af_id > 0) {
            ...
            if ($session_id > 0) {
                ...
                $sessions_ids = Session::select('id')->where([['af_id', $af->id], ['training_site', $training_site]])
                ->whereBetween('started_at', [$start_date, $end_date])
                ->pluck('id');
            } else {
                //$sessions=Session::select('id')->where('af_id',$af->id)->get();
                $sessions_ids = Session::select('id')->where('af_id', $af->id)
                ->whereBetween('started_at', [$start_date, $end_date])
                ->pluck('id');
            }
                ...
                if (count($intra_ids_entities) > 0) {
                    foreach ($intra_ids_entities as $entity_id) {
                        ...
                        $intra_ids_sessions = DB::table('af_schedulecontacts')
                            ...
                            ->whereBetween('af_sessiondates.planning_date', [$start_date, $end_date])
    ...
    public function manageTaskSousTask($data)
    {
        if (count($data) > 0) {
            $task = new Task();
            $comment = new Comment();
            ...
            $schedules = DB::table('af_schedules')
                ...
                ->join('af_sessions', 'af_sessions.id', '=', 'af_sessiondates.session_id')
                ->join('af_actions', 'af_actions.id', '=', 'af_sessions.af_id')
                ->select('af_schedules.start_hour')
                ->where('af_actions.id', $data['aflistvalue'])
                ->orderBy('af_schedules.start_hour', 'desc');

            $session = $schedules->first();
            $ended_date = \Carbon\Carbon::parse($session->start_hour);
            
            if ($start_date < $ended_date->copy()->subDays(15)) {
                $callback_date = $ended_date->copy()->subDays(15)->format('Y-m-d H:i:s');
            } else {
                $callback_date = $start_date->format('Y-m-d H:i:s');
            }
            
            $task->start_date = $start_date->format('Y-m-d H:i:s');
            $task->ended_date = $ended_date->format('Y-m-d H:i:s');
            ...
            $task->etat_id = (isset($data['etatvalue'])) ? $data['etatvalue'] : $valide_etat;
            $task->priority = (isset($data['prioritetext'])) ? $data['prioritetext'] : null;
            $task->responsable_id = (isset(auth()->user()->contact_id ) ?auth()->user()->contact_id : auth()->user()->id);
            $task->apporteur_id = (isset(auth()->user()->contact_id ) ? auth()->user()->contact_id: auth()->user()->id);
        ...
        return ['task' => $task_id, 'comment' => $comment_id, 'data' => $task_data];
    }
    
    public function manageSousTask($data)
    {
        try{
            $subtask = new Task();
            $start_date = \Carbon\Carbon::now();
            $schedules = DB::table('af_schedules')
                ...
                ->join('af_sessions', 'af_sessions.id', '=', 'af_sessiondates.session_id')
                ->join('af_actions', 'af_actions.id', '=', 'af_sessions.af_id')
                ->select('af_schedules.start_hour','af_schedules.end_hour','af_schedules.duration','af_sessions.code','af_sessions.title')
                ->where('af_actions.id', $data['af_id'])
                ->orderBy('af_schedules.start_hour', 'desc');
    
            $scheduleData = $schedules->get();
            $ended_date = \Carbon\Carbon::parse($schedules->first()->start_hour);
            ...
            $subtask->start_date = $start_date->format('Y-m-d H:i:s');
            $subtask->ended_date = $ended_date->format('Y-m-d H:i:s');
            $subtask->callback_date = $callback_date ;
            $subtask->task_parent_id = isset($data['id']) ? $data['id'] : null; 
            $subtask->sub_task = 1 ;
            $contact = Contact::find($data['contact_id']);
            $fullname = $contact ? $contact->first_name . ' ' . $contact->last_name : '';
    
            $subtask->title = isset($data['title']) ? $data['title'] . ' ' . $fullname : null;
            ...
            if (!$scheduleData->isEmpty()) {
                foreach ($scheduleData as $schedule) {
                    $output .= "Concerne la séance : {$schedule->code} {$schedule->title} du: {$schedule->start_hour}, à: {$schedule->end_hour}, de: {$schedule->duration} h <br/>";
                }
            }
            
            $subtask->description = (isset($output)) ? $output : null;
            $subtask->source_id = (isset($data['source_id'])) ? $data['source_id'] : null;
            $subtask->type_id = (isset($data['type_id'])) ? $data['type_id'] : null;
    
            $valide_etat = Param::where([['param_code', 'Etat'], ['code', 'Validé'], ['is_active', 1]])->pluck('id')->first();
    
            $subtask->etat_id = (isset($data['etat_id'])) ? $data['etat_id'] : $valide_etat;
            $subtask->priority = (isset($data['priority'])) ? $data['priority'] : null;
            $subtask->responsable_id = (isset(auth()->user()->contact_id ) ?auth()->user()->contact_id : null);
            $subtask->apporteur_id = (isset(auth()->user()->contact_id ) ? auth()->user()->contact_id: null);
    
    
            $subtask->callback_mode = (isset($data['callback_mode'])) ? $data['callback_mode'] : null;
            $subtask->reponse_mode_id = (isset($data['reponse_mode_id'])) ? $data['reponse_mode_id'] : null;
            $subtask->af_id = (isset($data['af_id'])) ? $data['af_id'] : null;
            $subtask->pf_id = (isset($data['pf_id'])) ? $data['pf_id'] : null;
            $subtask->entite_id = (isset($data['entite_id'])) ? $data['entite_id'] : null;
            $subtask->contact_id = (isset($data['contact_id'])) ? $data['contact_id'] : null;
            $subtask->is_read = (isset($data['is_read'])) ? $data['is_read'] : null;
    
            if (isset($data['is_sent']))
                $subtask->is_sent = $data['is_sent'];
            if (isset($data['file']))
                $subtask->file = $data['file'];
            if (isset($data['task_parent_id']))
                $subtask->task_parent_id = $data['task_parent_id'];
            if (isset($data['sub_task']))
                $subtask->sub_task = $data['sub_task'];
     ...
     * code auxiliaire
     */
    public function generateAuxiliaryAccountForEntity($entity_id,$address)
    {
        ...
        if ($entity->entity_type == 'P') {
            ...
            if ($address) {
                ...
                $city = $this->removeAccentsAndSpecial($address, false /* remain spaces */);
