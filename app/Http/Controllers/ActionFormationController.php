...
class ActionFormationController extends Controller
{
    ...
    public function constructViewContentAf($viewtype, $row_id)
    {
        ...
        $count = count($tasks);
        return view('pages.af.construct.view', compact('row', 'viewtype', 'sessions', 'af_sheet_id', 'lieux', 'groups', 'members', 'intervenant','count')); //,'enrollments'
    }
                            ...
                            if (count($data['schedules']) > 0) {
                                foreach ($data['schedules'] as $ds) {
                                    if (!empty($ds['start_hour']) && !empty($ds['end_hour'])) {
                                        ...
                                        $start_hour = $planning_date->format('Y-m-d') . ' ' . $ds['start_hour'] . ':00';
                                        $end_hour = $planning_date->format('Y-m-d') . ' ' . $ds['end_hour'] . ':00';

                                        // $start_hour = Carbon::createFromFormat('Y-m-d H:i:s', $start_hour);
                                        // $end_hour = Carbon::createFromFormat('Y-m-d H:i:s', $end_hour);

                                        $schedule = Schedule::where('start_hour', $end_hour)->get();

                                        if (count($schedule) > 0) {
                                            $carbonDateTime = Carbon::parse($end_hour_prev);
                                            ...
                                            $end_hour_prev = $inputDatetime_end->format('Y-m-d H:i:s');
                                        } else {
                                            $end_hour_prev = $end_hour;
                                        }


                                        // if($start_hour == $end_hour_prev)
                                        // {
                                        //     $carbonDateTime = Carbon::parse($end_hour_prev);
    ...
    {
        // dd($request,$request->has('file_id'));
        $success = false;
        $msg = 'Veuillez vérifier tous les champs du fomulaire !';
        if ($request->isMethod('post')) {
            ...
            if ($enrollment_id > 0) {
                ...
                // dd($request);
                $ids = Member::select('id')->where([['contact_id', '>', 0], ['enrollment_id', $enrollment_id]])->pluck('id');
                //     $selectedstudents = Member::select('id')
                //         ->whereIn('id', $request->members) // Further filter by selected student IDs
                //         ->pluck('id');

                //         $membersCount = DB::table('af_members')
                // ->join('en_contacts', 'en_contacts.id', '=', 'af_members.contact_id')
                // ->where('en_contacts.entitie_id', $request->entitie_id)
                // ->get();

                //         // dd($request->members,$ids,$membersCount);
                $datas = Contact::where([['entitie_id', $request->entitie_id]])->pluck('id');

                $newdata = [];

                $schedules_contacts = [];

                //     $ids2 = Member::select('id')->where([['enrollment_id', $request->entitie_id],['enrollment_id', $enrollment_id]])->pluck('id');

                $selectedStudentIds = $request->members;

                $unselectedStudentIds = $datas->diff($selectedStudentIds);

                // dd($request,$ids,$datas,$unselectedStudentIds);


                // foreach($unselectedStudentIds as $id => $value)
                //     {
                //         $rs_mb = Member::select('id')->where([['contact_id', $value], ['enrollment_id', $enrollment_id]])->pluck('id');

                //         $schedules_contacts[] = Schedulecontact::where('member_id',$rs_mb)->get();
                ...
                //     }
                //     dd($ids,$newdata,$schedules_contacts);



                // dd($unselectedStudentIds);
                ...
                // dd($ids,$ids,$membersCount,$datas);
                // dd($ids,$request->members);
                // if (count($ids) > 0) {
                //     $deletedRows = $DbHelperTools->massDeletes($ids, 'member', 1);
                // }
                // dd(count($request->members),count($ids));
    ...
    public function treeSchedulesContactsTest($af_id)
    {
        $DbHelperTools = new DbHelperTools();
                            ...
                            foreach ($schedules as $schedule_id => $schedule_datas) {
                                ...
                                if ($mode == 'withcontacts') {
                                    ...
                                    if (count($schedulecontacts) > 0) {
                                        ...
                                        $groupIdsList = $session_datas['groupsIds'];

                                        foreach ($groupIdsList as $key => $value) {
                                            ...
                                            $membersall = Member::where('group_id', $value)->get();

                                            foreach ($membersall as $member) {
                                                $memeber_fullname = $member->contact->firstname . ' ' . $member->contact->lastname;

                                                $datas[] = array(
                                                    "id" => 'SCONTACT',
                                                    "text" => $memeber_fullname,
                                                    "state" => array('opened' => true, 'checkbox_disabled' => true),
                                                    "icon" => "fa fa-user text-dark",
                                                    "parent" => 'SCHEDULE' . $schedule_id
                                                );
    ...
    public function sdtEnrollmentsMembers(Request $request, $af_id)
    {
        ...
        foreach ($datas as $d) {
            ...
            //Actions
            $type = "'MEMBER'";
            $btn_delete = '<button class="btn btn-sm btn-clean btn-icon" onclick="_deleteEnrollmentMember(' . $d->id . ',' . $type . ')" title="Suppression"><i class="' . $tools->getIconeByAction('DELETE') . '"></i></button>' . $btn_devis .''. $btn_devis_recue;
            $row[] = $btn_delete;
    ...
    public function storeFormScheduleRessource(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // dd($schedules_ids,$ressources_ids);
            ...
            if (count($schedules_ids) > 0 && count($ressources_ids) > 0) {
                foreach ($schedules_ids as $schedule_id) {
                    ...
                    foreach ($ressources_ids as $ressource_id) {
                        ...
                        //    dd($rsCheckParent,$can_be_scheduled,$ressource_id,$schedule_id);
        ...
        // dd($messagesParent,$messages,$msg);

        // dd($messages,$messagesParent);
        ...
        // dd($success,$msg,$messagesParent,$messages,$html);
    ...
    public function sdtContracts(Request $request, $af_id)
    {
        ...
        if ($roles[0]->code == 'FORMATEUR') {
            ...
            $contactid = DB::table('users')->where('id', $userid)->pluck('contact_id');
            ...
            if (count($enrollments_ids) > 0) {
                $ids_contacts = Member::select('id')->whereIn('enrollment_id', $enrollments_ids)->pluck('id');
            }
            if (count($ids_contacts) > 0) {
                // dd($ids_contacts);
                $ids_contracts = Schedulecontact::select('contract_id')->whereIn('member_id', $ids_contacts)->pluck('contract_id');
                $datas = Contract::whereIn('id', $ids_contracts)->orderBy('id', 'desc')->get();
            }
            foreach ($datas as $d) {
                $row = array();
                $spanAfName = '';
                ...
                $btn_attached_docs = '<a href="javascript:void(0)" onclick="_modalAttachedDocsContract(' . $d->id . ',' . $rsAfInfos['af_id'] . ')" class="navi-link"><span class="navi-icon"><i class="' . $tools->getIconeByAction('VIEW') . '"></i></span><span class="navi-text">Documents attachés</span></a>';
                $btn_sign_docs = $d->state != 'SC_GENERATED' || Auth::user()->roles->filter(function ($r) {
                    return $r->profil->code == 'CRFPE';
                })->isEmpty()
                    ? ''
    ...
    public function formselectionafintervenant($member_id)
    {
        ...
        // dd($id_contact,$ids_schedule,$member_id);
    ...
    public function storeFormPointage(Request $request)
    {
        ...
        $schedule = Schedule::find($schedulecontact->schedule_id);

        if ($schedule->start_hour > Carbon::now()) {
            return response()->json([
                'success' => false,
                'msg' => 'Impossible de mettre à jour avant la date de début prévue.',
                'error_code' => 'START_DATE_PASSED',
            ]);
        }
                    ...
                    if ($result) {
                        ...
                        try {
                            ...
                            if ($pointing == 'absent') {
                                ...
                                Mail::send('pages.email.model', ['htmlMain' => $content, 'htmlHeader' => $header, 'htmlFooter' => $footer], function ($m) {
                                    $m->from(auth()->user()->email);
                                    $m->bcc([auth()->user()->email, 'karjane@havetdigital.fr']);
    ...
    public function storeFormBulkPointage(Request $request)
    {
        $success = true;
        $msg = 'L\'affectation a été enregistrée avec succès';
        try {
            $uniqueScheduleIds = array_unique($request->schedule_ids);
            Schedulecontact::whereIn('id', $uniqueScheduleIds)->update(['pointing' => 'present']);
        } catch (\Exception $e) {
            $success = false;
            $msg = 'L\'affectation n\'a pas été enregistrée. Erreur : ' . $e->getMessage();
        }
        return response()->json([
            'success' => $success,
            'msg' => $msg,
